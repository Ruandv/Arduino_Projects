...
String results[100] = {"Confucius say, virginity like bubble. One prick - all gone",
...
"Confucius say, electrician get much angry when find shorts in wife",
...
BLYNK_WRITE(V0)
{
  ...
  {
    WriteMessage(results[random(0,100)]);
    lastTick = millis();
  }
}
...
BLYNK_WRITE(V2)
{
  ...
  if (String("Tweet") == param.asStr() || String("tweet") == param.asStr()) {
    WriteMessage("What would you like to Tweet'") ;
    EnableTweetFlags();
  } 
  else if(pendingTweet == true) 
  {    
    TweetWriteMessage( param.asStr() );
    WriteMessage("Go check your twitter account!");
  }
