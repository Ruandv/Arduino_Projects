...
Set-Variable -Name programVersion -Value (Get-Content $ProgramAuthTokenFile| Select -Index 3)
$programVersion = $programVersion -replace '[a-z,=,*,;,",\s+]',''
Write-Host $FireBaseArduino.FullName
Write-Host $programVersion 
Set-Variable -Name FirebaseBindary -Value (Get-ChildItem -Path $FireBaseArduino.FullName -filter "*Blynk.ino.bin" -File -Force -ErrorAction SilentlyContinue | Select-Object FullName  -First 1)
Copy-Item $FirebaseBindary.FullName C:\Temp\$($programVersion)_Nursery_Blynk.ino.bin
